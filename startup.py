...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        default="models/ego_blur_face.jit",
...
def visualize(
    ...
    """
    Visualize the input image with an out-of-focus effect applied to detections.

    Parameters:
        image: np.ndarray
            Image on which we want to apply the effect.
        detections: List[List[float]]
            List of bounding boxes in format [x1, y1, x2, y2].
        scale_factor_detections: float
            Scale detections by the given factor to allow blurring more area.
            For example, 1.15 means 15% scaling.

    Returns:
        np.ndarray: Image with out-of-focus effect applied to specified areas.
    """
    ...
    for box in detections:
        # Scale the bounding box if needed
        ...
        # Draw an elliptical mask over the blurred region
    ...
    # Combine the background and foreground to produce the final result
    result_image = cv2.add(image_bg, image_fg)
...
if __name__ == '__main__':
    ...
    logging.info(f"NMS iou threshold: {args.nms_iou_threshold}")
    logging.info(f"Scale factor detections: {args.scale_factor_detections}")
    logging.info(f"Input: {args.input_image_path }")
